...
const Pelis = ({ isLoggedIn, setIsLoggedIn }) => {
    ...
    async function getTopMoviesOnNetflix() {
      try {
        ...
        const response = await Promise.all([
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=es-ES&region=ES&page=${page}`),
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=es-ES&region=ES&page=${page+1}`),
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${apiKey}&language=es-ES&region=ES&page=${page+2}`)
        ]);
        ...
        for (const movie of topMovies) {
          const movieResponse = await fetch(`https://api.themoviedb.org/3/movie/${movie.id}/watch/providers?api_key=${apiKey}`);
  ...
  const filteredMovies = topMovies.filter((movie) => {
    ...
    if (yearFilter.start && new Date(movie.release_date).getFullYear() < parseInt(yearFilter.start)) {
      return false;
    }
    if (yearFilter.end && new Date(movie.release_date).getFullYear() > parseInt(yearFilter.end)) {
      return false;
    }
    return true;
  });
        ...
        {filteredMovies.map((movie, index) => (
          <div key={movie.id} className="movie-container">
            <div className="movie-info-left">
              ...
              <img className="movie-poster" src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt={`Poster de ${movie.title}`} />
            </div>
            <div className="movie-info-right">
              ...
              <p className="movie-release-date">Año de lanzamiento: {movie.release_date.slice(0, 4)}</p>
            </div>
          </div>
        ))}
